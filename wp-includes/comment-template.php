...
function comment_form( $args = array(), $post_id = null ) {
	...
	$fields   =  array(
		'author' => '<p class="comment-form-author">' . '<label for="author" required>' . __( '' ) . ( $req ? ' <span class="required" >*</span>' : '' ) . '</label> ' .
		            '<input id="author" name="author" type="text" size="30" placeholder="Your Name"' . $aria_req . $html_req . ' /></p>',
		'url'    => '<p class="comment-form-url"><label for="url">' . __( 'Website' ) . '</label> ' .
		            '<input id="url" name="url" ' . ( $html5 ? 'type="url"' : 'type="text"' ) . ' size="30" /></p>',
	);
	...
	$defaults = array(
		'fields'               => $fields,
		'comment_field'        => '<p class="comment-form-comment"><label for="comment">' . _x( '', 'noun' ) . '</label> <textarea id="comment" name="comment" cols="45" rows="8"  aria-required="true"  required="required" placeholder="Question/Concern"></textarea></p>',
	...
	if ( comments_open( $post_id ) ) : ?>
		...
		<div id="respond" class="comment-respond">
			<?php
			echo '<hr />' . $args['title_reply_before'];
